...
- **Runtime page: netdisk mirror entry (placeholder)**: a new "Netdisk" button (`CloudDownload` icon) is added to the header of the "Download runtime" card, sitting to the left of the existing Refresh button. Clicking it routes through `invoke('open_url')` (with a `window.open` fallback) to the maintainer's netdisk mirror — useful for users in mainland China where GitHub release assets are slow even after the proxy support. The URL is held in a single `NETDISK_DOWNLOAD_URL` constant at the top of `src/pages/Runtime.tsx`; it currently reads `https://pan.baidu.com/s/PLACEHOLDER_NEEDS_FILL` and **needs to be replaced with the real share link** before release. New i18n keys: `netdiskDownload`, `netdiskTitle`.
