# llama-server(1) — Debian Manpages (English)

llama-server manual page, English original, saved from:

- https://manpages.debian.org/unstable/llama.cpp-tools/llama-server.1.en.html
- Source file: llama-server.1.en.gz (from llama.cpp-tools 1:0.2.0+dfsg1-1)
- Source last updated: 2026-08-23T07:06:00Z
...
-t, --threads N
:   number of CPU threads to use during generation (default: <num_cpus>) (env: LLAMA_ARG_THREADS)
...
:   DEPRECATED in favor of `--load-mode`: use DirectIO if available (env: LLAMA_ARG_DIO)
...
:   attempt optimizations that help on some NUMA systems:
    ...
    see https://github.com/ggml-org/llama.cpp/issues/1437 (env: LLAMA_ARG_NUMA)
...
:   how to split the model across multiple GPUs, one of:
    ...
    - tensor: split weights and KV across GPUs (parallelized, EXPERIMENTAL) (env: LLAMA_ARG_SPLIT_MODE)
...
:   comma-separated list of types of speculative decoding to use (default: none) (env: LLAMA_ARG_SPEC_TYPE)
...
:   max number of context checkpoints to create per slot (default: 32) [(more info)](https://github.com/ggml-org/llama.cpp/pull/15293) (env: LLAMA_ARG_CTX_CHECKPOINTS)
...
:   device to use for multimodal projector (none = don't offload, default: auto). use --list-devices to see a list of available devices (env: MTMD_BACKEND_DEVICE)
...
:   experimental: run tools in a separate runtime environment (default: none, use host environment). available options:
...
:   controls whether thought tags are allowed and/or extracted from the response, and in which format they're returned; one of:
    ...
    - deepseek-legacy: keeps ` thinking` tags in `message.content` while also populating `message.reasoning_content`
...
:   reasoning effort level given to the chat template: 'default' to keep the template default, or a level such as 'minimal', 'low', 'medium', 'high', 'xhigh' or 'max' (default: default) (env: LLAMA_ARG_REASONING_EFFORT)
...
Source file: llama-server.1.en.gz (from llama.cpp-tools 1:0.2.0+dfsg1-1), Debian unstable